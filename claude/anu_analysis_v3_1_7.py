...
def full_bot_source_paths(path: Path) -> List[Path]:
    """Return the one complete bot ZIP accepted by this engine."""
    if re.match(r"^(?:bot-eod-report|eod-flow-report|flow-eod-report)-\d{4}-\d{2}-\d{2}\.part-\d+-of-\d+\.(?:csv|zip)$", path.name):
        raise FileNotFoundError(f"Split bot EOD files are not accepted: {path}")
    return [path]
