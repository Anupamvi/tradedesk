...
Persistent dashboard: auto-starts in Cloud Agents (`python3 -m compoundcore dashboard` on port **8765** via `.cursor/environment.json`). On your Mac, agents run `compoundcore/scripts/install-macos-dashboard-service.sh` once so it survives reboot — then open `http://127.0.0.1:8765/`. Type a dollar amount to split **both** sleeves. In **My book**, save cost vs now; **Refresh** pulls Schwab quotes and sleeve lots and shows real gain or loss.
