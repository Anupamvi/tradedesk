"""Schwab tape: quotes, history, chains. Market Data GET only. Never place trades.

Token stays at SCHWAB_TOKEN_PATH from tradedesk/.env (or groat/.env). This desk
does not copy tokens into trendbook/.
"""
...
from trendbook.config import CODE_DIR, LOOKBACK_DAYS
...
def read_bars_cache(ticker: str) -> List[dict]:
    cache_path = schwab_cache_dir() / ("%s.json" % str(ticker).upper())
    if not cache_path.is_file():
        return []
    try:
        payload = json.loads(cache_path.read_text(encoding="utf-8"))
        return [b for b in (payload.get("data") or []) if isinstance(b, dict) and b.get("date")]
...
def price_history_bars(
    ...
    cached = read_bars_cache(name) if use_cache else []
    if not live:
    ...
    try:
        ...
        asof_d = datetime.now(timezone.utc).replace(tzinfo=None)
...
def _parse_quote(wrap: dict, asof: str) -> Optional[dict]:
    ...
    return {
        ...
        "high52": to_float(q.get("52WeekHigh") or q.get("high52")),
...
MOVER_INDEXES = ("$DJI", "$COMPX", "$SPX.X", "$RUT.X", "$NDX.X")
