"""Load env from trendbook/.env, then xhigh/.env, groat/.env, tradedesk/.env. Never print values."""

import os
from pathlib import Path
from typing import Dict, Mapping, Optional

from trendbook.config import CODE_DIR, TRADEDESK_ENV, TRADEDESK_ROOT

ORATS_TOKEN_MISSING = (
    "ORATS_TOKEN missing. Put it in xhigh/.env, groat/.env, or export ORATS_TOKEN."
)
...
def load_merged_env(code_dir: Optional[Path] = None) -> Dict[str, str]:
    ...
    search = (
        root / ".env",
        TRADEDESK_ROOT / "xhigh" / ".env",
        TRADEDESK_ROOT / "groat" / ".env",
        TRADEDESK_ENV,
    )
    for path in search:
...
def load_orats_token(
    ...
    if token_file:
        ...
        if path.is_file():
            found = _token_from_text(path.read_text(encoding="utf-8"), dotenv=False)
            if found:
                return found
...
def schwab_credentials(code_dir: Optional[Path] = None) -> Optional[Dict[str, str]]:
    env = load_merged_env(code_dir=code_dir)
