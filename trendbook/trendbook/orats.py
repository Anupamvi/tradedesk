"""ORATS delayed Data API v2. Copied from xhigh, plus strikes for ADD overlay. Never log the token."""
...
from trendbook.dates import parse_any_date
...
def fetch_cores(
    ...
    out: Dict[str, dict] = {}
    hist = bool(asof and today and asof != today)
    ...
    for i in range(0, len(names), ORATS_TICKER_BATCH):
        ...
        query = {"ticker": ",".join(group), "fields": fields}
        if hist:
            query["tradeDate"] = asof
        status, payload, err = http_get(path, query, token)
...
def parse_core(row: Optional[dict]) -> Dict[str, Any]:
    ...
    return {
        ...
        "px_chng_1m": to_float(row.get("stkPxChng1m")),
        "px_chng_6m": to_float(row.get("stkPxChng6m")),
        "px_chng_1y": to_float(row.get("stkPxChng1y")),
        "raw": row,
    }


def cheap_iv(core: Optional[dict]) -> bool:
    if not core:
        return False
    from trendbook.config import IV_HV_CHEAP, IV_PCTILE_CHEAP

    pctile = to_float(core.get("iv_pctile_1y") if "iv_pctile_1y" in core else core.get("ivPctile1y"))
    iv_hv = to_float(core.get("iv_hv") if "iv_hv" in core else core.get("ivHvXernRatio"))
    if pctile is None or iv_hv is None:
        return False
    return pctile < IV_PCTILE_CHEAP and iv_hv < IV_HV_CHEAP
