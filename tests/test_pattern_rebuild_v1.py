...
import uwos.pattern_rebuild_v1.core as pattern_core
import uwos.pattern_rebuild_v1.managed as managed_module
from uwos.pattern_rebuild_v1.core import (
    PriceRow,
    PriceSignal,
    SourceRef,
...
from uwos.pattern_rebuild_v1.managed import (
    ManagedConfig,
    QuoteCache,
    _signals_for_day,
...
def test_predeclared_selection_uses_only_earlier_validation_and_marks_selected_candidate():
    ...
    def candidate_rows(name, selection_value, holdout_value):
        rows = []
        for index, day in enumerate(dates):
...
def test_managed_ticket_state_does_not_execute_stale_historical_quotes():
    stale = pattern_core._managed_ticket_state(
        {"entry_date": "2026-08-19"},
        {"status": "QUALIFIED_MANAGED", "approval_status": "QUALIFIED_MANAGED"},
        "2026-08-20",
    )
    assert stale == (
...
def test_managed_report_separates_historical_rows_from_live_action_board():
    ...
    assert "NO_LIVE_EXECUTION_CANDIDATE" in report
    assert "Historical Backtest References" in report
    assert "HISTORICAL_ONLY | BCO" in report
    assert "TRADE_REVIEW | MRK" not in report.split("## Historical Backtest References", 1)[0]
