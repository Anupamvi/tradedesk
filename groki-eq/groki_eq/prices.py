"""Daily OHLC tape. Cache first, then Schwab history, then one ORATS series pull."""

from typing import Dict, List, Optional

from groki_eq.orats import (
...
def rows_to_bars(rows: List[dict]) -> List[dict]:
    ...
    for row in rows or []:
        ...
        close = _to_float(row.get(keys["close"]))
        high = _to_float(row.get(keys["high"]))
        low = _to_float(row.get(keys["low"]))
        open_ = _to_float(row.get(keys["open"]))
        if len(day) != 10 or close is None or high is None or low is None:
            continue
        bars.append({"date": day, "open": open_, "high": high, "low": low, "close": close})
...
def load_cached_bars(ticker: str) -> List[dict]:
    ...
    if payload is None:
        return []
    return rows_to_bars(_rows(payload))
...
def ensure_bars(
    ...
    if cached and not stale:
        return {"bars": cached, "tape": "orats_cache", "http": 0, "error": ""}
    if not token:
        return {"bars": cached, "tape": "orats_cache" if cached else "missing", "http": 0, "error": "" if cached and not stale else "missing_bars"}
    pack = fetch_dailies_series(
        name, token, getter=getter, max_requests=max_requests, refresh=stale
    )
    ...
    if extra:
        ...
        tape = "schwab_history"
    if not bars:
        return {"bars": [], "tape": tape, "http": pack.get("http") or 0, "error": pack.get("error") or "missing_bars"}
