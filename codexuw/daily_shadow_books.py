...
DEBIT_SHADOW_POLICY_VERSION = "debit-walkforward-shadow-v3-live-guard-20260816"
DEBIT_TRAINING_FILE = "codexdaily_v4_debit_shadow_training_v2_2026-08-14.csv.gz"
DEBIT_PILOT_VALIDATION_FILE = "codexdaily_v4_debit_bull_call_pilot_validation_v2_2026-08-16.json"
...
def prepare_live_debit_candidates(scored: pd.DataFrame, *, asof: dt.date) -> pd.DataFrame:
    """Create point-in-time feature rows from unresolved live debit candidates."""
    ...
    if source.empty:
        return source
    source["_v4_source_index"] = source.index
    ...
    candidate["next_earnings_dt"] = pd.to_datetime(
        _series(source, "next_earnings_dt", pd.NaT), errors="coerce"
    )
    ...
    # prepare_history owns the shared feature formulas. These neutral placeholders
    # support those formulas only; live_candidate_guard never treats them as
    # eligibility evidence, model inputs, labels, or observed outcomes.
    ...
    if prepared.empty:
        return prepared
    if "earnings_known" not in prepared.columns:
        next_earnings = pd.to_datetime(
            _series(prepared, "next_earnings_dt", pd.NaT), errors="coerce"
        )
        entry_day = pd.to_datetime(_series(prepared, "entry_day", pd.NaT), errors="coerce")
        expiry = pd.to_datetime(_series(prepared, "expiry", pd.NaT), errors="coerce")
        prepared["earnings_known"] = next_earnings.notna()
        prepared["earnings_crosses"] = prepared["earnings_known"] & next_earnings.between(
            entry_day, expiry
        )
    ...
    guard = debit_model.live_candidate_guard(prepared)
...
def apply_live_debit_execution_model(
    scored: pd.DataFrame,
    *,
    root: Path,
    asof: dt.date,
    allow_execution_authority: bool,
) -> tuple[pd.DataFrame, dict[str, Any]]:
    """Attach leakage-safe debit predictions before production disposition."""
    out = scored.copy()
    defaults: dict[str, Any] = {
        "debit_wf_policy_version": "",
        "debit_wf_predicted_win_probability": np.nan,
        "debit_wf_expected_value": np.nan,
        "debit_wf_prior_sample_size": 0,
        "debit_wf_model_training_through": "",
        "debit_wf_live_guard_pass": False,
        "debit_wf_production_authorized": False,
        "debit_wf_authority_scope": "",
    }
    for column, default in defaults.items():
        out[column] = default

    validation_path = root / "codexuw" / "history" / DEBIT_PILOT_VALIDATION_FILE
    try:
        evaluated, _, model_summary = score_debit_shadow(out, root=root, asof=asof)
        ...
        return out, {
            ...
            "production_candidate_rows": 0,
        }
    validation = json.loads(validation_path.read_text()) if validation_path.exists() else {}
    activation_value = pd.to_datetime(validation.get("activation_date"), errors="coerce")
    activation = None if pd.isna(activation_value) else activation_value.date()
    validation_pass = bool(
        validation.get("status") == "PASS"
        and validation.get("execution_authorized") is True
        and activation is not None
        and dt.date.today() >= activation
        and allow_execution_authority
    )
    live_scope = validation.get("live_quality_scope") or {}
    minimum_reward_risk = float(live_scope.get("minimum_reward_risk", 1.25))
    maximum_quote_width = float(live_scope.get("maximum_quote_width_pct", 0.25))
    allowed_oi_status = {
        str(value).strip().lower()
        for value in live_scope.get(
            "allowed_oi_status", ["supportive", "matched_unconfirmed", "mixed"]
        )
    }

    for _, candidate in evaluated.iterrows():
        source_index = candidate.get("_v4_source_index")
        if pd.isna(source_index) or source_index not in out.index:
            continue
        out.at[source_index, "debit_wf_policy_version"] = DEBIT_SHADOW_POLICY_VERSION
        out.at[source_index, "debit_wf_predicted_win_probability"] = candidate.get(
            "predicted_win_probability"
        )
        out.at[source_index, "debit_wf_expected_value"] = candidate.get(
            "predicted_ev_payoff_correct"
        )
        out.at[source_index, "debit_wf_prior_sample_size"] = candidate.get(
            "prior_sample_size", 0
        )
        out.at[source_index, "debit_wf_model_training_through"] = candidate.get(
            "model_training_through", model_summary.get("training_through", "")
        )
        out.at[source_index, "debit_wf_live_guard_pass"] = True
        strategy = str(candidate.get("strategy") or "")
        probability = float(candidate.get("predicted_win_probability") or 0.0)
        expected_value = float(candidate.get("predicted_ev_payoff_correct") or 0.0)
        reward_risk = float(candidate.get("reward_risk") or 0.0)
        quote_width = float(candidate.get("entry_quote_width_pct") or np.inf)
        oi_status = str(candidate.get("oi_carryover_status") or "").strip().lower()
        strict_bull_pilot = bool(
            validation_pass
            and strategy == "Bull Call Debit Spread"
            and probability >= 0.55
            and expected_value > 0
            and reward_risk >= minimum_reward_risk
            and quote_width <= maximum_quote_width
            and oi_status in allowed_oi_status
        )
        out.at[source_index, "debit_wf_production_authorized"] = strict_bull_pilot
        if strict_bull_pilot:
            out.at[source_index, "debit_wf_authority_scope"] = validation.get(
                "authority_scope", "one_contract_bull_call_probability_gte_0.55_positive_ev"
            )

    summary = dict(model_summary)
    model_policy_version = summary.get("policy_version", DEBIT_SHADOW_POLICY_VERSION)
    summary.update(
        {
            "model_policy_version": model_policy_version,
            "policy_version": validation.get("policy_version", model_policy_version),
            "validation_path": str(validation_path),
            "validation_status": validation.get("status", "MISSING"),
            "activation_date": validation.get("activation_date", ""),
            "execution_authority_enabled": validation_pass,
            "execution_authorized": validation_pass,
            "production_authorized": validation_pass,
            "shadow_only": not validation_pass,
            "reason": (
                "validated one-contract bull-call debit production lane active"
                if validation_pass
                else summary.get("reason", "debit model remains research-only")
            ),
            "authority_scope": validation.get("authority_scope", ""),
            "production_candidate_rows": int(
                out["debit_wf_production_authorized"].astype(bool).sum()
            ),
        }
    )
    return out, summary
...
_write_daily_shadow_outputs_v2 = write_daily_shadow_outputs
...
def write_daily_shadow_outputs(
    ...
    *,
    ...
    artifacts, summary = _write_daily_shadow_outputs_v2(
        scored,
        out_dir=out_dir,
        root=root,
        asof=asof,
    ...
    try:
        from codexuw.debit_medium_shadow_v3 import write_live_medium_outputs

        _, medium_artifacts, medium_summary = write_live_medium_outputs(
            scored,
            out_dir=out_dir,
            root=root,
            asof=asof,
        ...
        artifacts.update(medium_artifacts)
        summary["debit_medium_bull_shadow_v3"] = medium_summary
    except Exception as exc:
        error_path = out_dir / f"debit_medium_bull_shadow_v3_error_{asof.isoformat()}.json"
        error_payload = {
            "policy_version": "codexdaily-v4-debit-medium-bull-shadow-v3-20260816",
            "execution_authorized": False,
            "error": f"{type(exc).__name__}: {exc}",
        }
        error_path.write_text(json.dumps(error_payload, indent=2) + "\n")
        artifacts["debit_medium_bull_shadow_v3_error"] = str(error_path)
        summary["debit_medium_bull_shadow_v3"] = error_payload
