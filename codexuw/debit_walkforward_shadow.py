...
def _candidate_economic_guard(frame: pd.DataFrame) -> pd.Series:
    return (
...
def candidate_guard(frame: pd.DataFrame) -> pd.Series:
    """Historical candidate guard with fully matured outcome integrity."""
    return learning_guard(frame) & _candidate_economic_guard(frame)


def live_candidate_guard(frame: pd.DataFrame) -> pd.Series:
    """Point-in-time debit guard that never requires future outcome fields."""
    fillable = (
        frame["exact_fillable"].map(_truthy)
        if "exact_fillable" in frame.columns
        else pd.Series(False, index=frame.index)
    )
    hard_reject_clear = (
        frame["hard_rejects"].fillna("").astype(str).str.strip().eq("")
        if "hard_rejects" in frame.columns
    ...
    point_in_time_integrity = (
        ...
        & frame["asof"].notna()
        & frame["entry_day"].notna()
        & frame["entry_day"].ge(frame["asof"])
    )
    return (
        point_in_time_integrity
        & frame["earnings_known"]
        & ~frame["earnings_crosses"]
        & _candidate_economic_guard(frame)
    )
