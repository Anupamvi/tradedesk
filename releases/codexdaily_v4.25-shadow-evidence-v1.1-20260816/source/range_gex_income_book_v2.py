...
def build_gex_collection_universe(scored: pd.DataFrame, *, max_tickers: int = 25) -> pd.DataFrame:
    """Prequalify range-income names before making any browser GEX requests."""
    source = scored[_series(scored, "strategy", "").astype(str).isin({"Bull Put Credit Spread", "Bear Call Credit Spread"})].copy()
    if source.empty:
        return pd.DataFrame(columns=["ticker", "collection_rank", "candidate_count", "reason"])
    credit = _num(source, "natural_credit")
    width = _num(source, "spread_width")
    credit_pct = credit / width.replace(0, np.nan)
    regime = _series(source, "symbol_regime_trend", "").fillna("").astype(str)
    regime = regime.where(regime.str.len().gt(0), _series(source, "regime_trend", "").astype(str))
    expiry = pd.to_datetime(_series(source, "expiry", ""), errors="coerce")
    earnings = pd.to_datetime(_series(source, "next_earnings_dt", ""), errors="coerce")
    quality = (
        _series(source, "live_status", "").astype(str).eq("PASS")
        & _series(source, "regular_session_quote", True).map(_truthy)
        & credit.gt(0)
        & width.gt(credit)
        & credit_pct.between(0.15, 0.45)
        & _num(source, "quote_width_pct").le(0.35)
        & _num(source, "dte").between(21, 44)
        & _num(source, "expected_move_ratio").le(0.90)
        & _num(source, "iv_hv_ratio").between(1.05, 2.50)
        & earnings.notna()
        & ~earnings.le(expiry)
        & regime.eq("range")
    )
    source = source.loc[quality].copy()
    if source.empty:
        return pd.DataFrame(columns=["ticker", "collection_rank", "candidate_count", "reason"])
    source["collection_rank"] = (
        2.0 * credit_pct.loc[source.index]
        - 0.8 * _num(source, "expected_move_ratio")
        - 0.5 * _num(source, "quote_width_pct")
        + 0.15 * _num(source, "iv_hv_ratio").clip(1.05, 2.50)
    )
    grouped = source.groupby(source["ticker"].astype(str).str.upper()).agg(
        collection_rank=("collection_rank", "max"),
        candidate_count=("strategy", "size"),
    ).reset_index().rename(columns={"ticker": "ticker"})
    grouped["reason"] = "range credit candidate passed price, IV/HV, DTE, earnings, and liquidity prefilter"
    return grouped.sort_values(["collection_rank", "candidate_count"], ascending=[False, False]).head(max(1, int(max_tickers))).reset_index(drop=True)
...
def write_prospective_outputs(scored: pd.DataFrame, *, out_dir: Path, root: Path, asof: dt.date) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    collection_path = out_dir / f"codexdaily_v4_range_gex_collection_universe_{asof}.csv"
    status_path = out_dir / f"codexdaily_v4_range_gex_shadow_status_{asof}.json"
    ledger_path = out_dir.parent / RANGE_LEDGER_NAME
    collection_universe = build_gex_collection_universe(scored)
    collection_universe.to_csv(collection_path, index=False)
    ...
    summary = {"policy_version": POLICY_VERSION, "status": status, "reason": reason, "shadow_only": True, "execution_authorized": False, "gex_rows": int(len(gex)), "collection_universe_rows": int(len(collection_universe)), "collection_universe_artifact": str(collection_path), "evaluated_rows": int(len(evaluated)), "qualified_rows": int(evaluated.get("range_gex_qualified", pd.Series(dtype=bool)).sum()) if not evaluated.empty else 0, "selected_vertical_rows": int(len(vertical)), "selected_condor_rows": int(len(condor)), "ledger_rows": int(len(ledger))}
    status_path.write_text(json.dumps(summary, indent=2, default=str) + "\n", encoding="utf-8")
    return {"range_gex_collection_universe": str(collection_path), "range_gex_shadow_candidates": str(evaluated_path), "range_gex_shadow_vertical": str(vertical_path), "range_gex_shadow_condor": str(condor_path), "range_gex_shadow_status": str(status_path), "range_gex_shadow_ledger": str(ledger_path)}, summary
