...
BROKER_OUTCOME_MATCH_AUDIT_COLUMNS = [
    ...
    "entry_symbols",
    "entry_leg_count",
    "matched_recommendation_count",
    "matched_trade_keys",
    "matched_report_dates",
    "matched_run_ids",
    "can_backfill_realized_pnl",
    "blocker",
    "source_path",
    "note",
]
...
PROFITABILITY_BUCKET_ATLAS_COLUMNS = [
    ...
    "liquidity_bucket",
    "status",
    "actual_bucket_status",
    "actual_bucket_sample_size",
    "actual_bucket_avg_pnl",
    "actual_bucket_profit_factor",
    "replay_bucket_status",
    "replay_bucket_sample_size",
    "replay_bucket_avg_pnl",
    "replay_bucket_profit_factor",
...
def output_paths(
    ...
    return {
        ...
        "outcome_evidence_audit": resolved_out / "outcome_evidence_audit.csv",
...
def run_pipeline(
    ...
    *,
    ...
    _write_frame(outcome_evidence_audit, paths["outcome_evidence_audit"])
...
def _write_dispatch_only_artifacts(
    ...
    _write_frame(pd.DataFrame(columns=OUTCOME_EVIDENCE_AUDIT_COLUMNS), paths["outcome_evidence_audit"])
    _write_frame(pd.DataFrame(columns=BROKER_OUTCOME_MATCH_AUDIT_COLUMNS), paths["broker_outcome_match_audit"])
    _write_frame(pd.DataFrame(columns=BROKER_MATCHED_OUTCOME_COLUMNS), paths["broker_matched_outcomes"])
    ...
    _write_frame(pd.DataFrame(columns=PROFITABILITY_BUCKET_ATLAS_COLUMNS), paths["profitability_bucket_atlas"])
...
def build_market_price_regime(
    ...
    *,
    ...
    mega_moves = [
        _as_float(item.get("move_pct"))
        for item in readings
        if item.get("ticker") in CORE_MEGA_CAP_TICKERS and _as_float(item.get("move_pct")) is not None
    ]
    avg_index_move = sum(index_moves) / len(index_moves) if index_moves else None
    avg_mega_move = sum(mega_moves) / len(mega_moves) if mega_moves else None
    mega_negative_breadth = (
        sum(1 for move in mega_moves if move <= -MACRO_TAPE_DIRECTIONAL_MOVE_PCT) / len(mega_moves)
        if mega_moves
        else None
    )
    mega_positive_breadth = (
        sum(1 for move in mega_moves if move >= MACRO_TAPE_DIRECTIONAL_MOVE_PCT) / len(mega_moves)
        if mega_moves
        else None
    )
    mega_breadth_conflict_min = 5
    mega_breadth_conflict_pct = 0.60
    ...
    if (
        tape_direction == "bullish"
        and len(mega_moves) >= mega_breadth_conflict_min
        and (
            (mega_negative_breadth is not None and mega_negative_breadth >= mega_breadth_conflict_pct)
            or (avg_mega_move is not None and avg_mega_move <= -MACRO_TAPE_DIRECTIONAL_MOVE_PCT)
        )
    ):
        tape_direction = "mixed"
        regime = "mixed"
        sizing = "normal"
        note = "index price tape is bullish but mega-cap breadth is bearish"
    elif (
        tape_direction == "bearish"
        and len(mega_moves) >= mega_breadth_conflict_min
        and (
            (mega_positive_breadth is not None and mega_positive_breadth >= mega_breadth_conflict_pct)
            or (avg_mega_move is not None and avg_mega_move >= MACRO_TAPE_DIRECTIONAL_MOVE_PCT)
        )
    ):
        tape_direction = "mixed"
        regime = "mixed"
        sizing = "normal"
        note = "index price tape is bearish but mega-cap breadth is bullish"
    ...
    return {
        ...
        "avg_mega_cap_move_pct": round(avg_mega_move, 6) if avg_mega_move is not None else None,
        "mega_cap_negative_breadth": round(mega_negative_breadth, 6) if mega_negative_breadth is not None else None,
        "mega_cap_positive_breadth": round(mega_positive_breadth, 6) if mega_positive_breadth is not None else None,
...
def annotate_macro_tape_candidates(raw_universe: pd.DataFrame, market_price_regime: Mapping[str, Any]) -> pd.DataFrame:
    ...
    if direction not in {"bearish", "bullish"}:
        return out
    threshold = _as_float(
        market_price_regime.get("directional_threshold_pct") if isinstance(market_price_regime, Mapping) else None
    )
    threshold = threshold if threshold is not None and threshold > 0 else MACRO_TAPE_DIRECTIONAL_MOVE_PCT
    ...
    ticker_move = pd.to_numeric(out["price_move_pct"], errors="coerce")
    ticker_direction = pd.Series("", index=out.index, dtype=object)
    ticker_direction = ticker_direction.mask(ticker_move.le(-threshold), "bearish")
    ticker_direction = ticker_direction.mask(ticker_move.ge(threshold), "bullish")
    effective_direction = ticker_direction.where(ticker_direction.ne(""), direction)
    out.loc[evaluated, "macro_tape_direction"] = effective_direction.loc[evaluated]
    opposite_ticker_tape = evaluated & ticker_direction.ne("") & ticker_direction.ne(direction)
    out.loc[evaluated & ~opposite_ticker_tape, "macro_tape_reason"] = (
        "core/mega-cap reviewed against " + direction + " price tape"
    )
    out.loc[opposite_ticker_tape, "macro_tape_reason"] = (
        "core/mega-cap reviewed against "
        + direction
        + " index tape; ticker move is "
        + ticker_direction.loc[opposite_ticker_tape]
    )
    promote = bias.eq("neutral") & tier.eq("core") & core_or_mega & score.ge(MACRO_TAPE_CANDIDATE_SCORE_FLOOR)
    if not promote.any():
        return out
    out.loc[promote, "macro_tape_direction"] = effective_direction.loc[promote]
    ...
    out.loc[promote, "bias"] = effective_direction.loc[promote]
    out.loc[promote, "macro_tape_reason"] = (
        "neutral UW flow rescued by "
        + effective_direction.loc[promote]
        + " price tape; original flow label preserved in flow_bias_label"
    )
    promoted_opposite = promote & opposite_ticker_tape
    out.loc[promoted_opposite, "macro_tape_reason"] = (
        "neutral UW flow rescued by "
        + effective_direction.loc[promoted_opposite]
        + " ticker tape despite "
        + direction
        + " index tape; original flow label preserved in flow_bias_label"
    )
...
def generate_candidates(
    ...
    *,
    ...
    if focus_tickers:
        ...
        ticker_series = raw_universe["ticker"].astype(str).str.upper()
        tier_series = raw_universe.get("underlying_quality_tier", pd.Series("", index=raw_universe.index)).astype(str).str.lower()
        focus = raw_universe[
            ticker_series.isin(focus_set)
            & tier_series.eq("core")
            & (
                raw_universe["bias"].ne("neutral")
                | macro_mask
                | macro_direction.isin(["bearish", "bullish"])
                | raw_universe["quality_status"].isin(["qualified", "watch"])
            )
        ].copy()
        if not focus.empty:
            core_review = (
                focus.get("bias", pd.Series("", index=focus.index)).astype(str).str.lower().eq("neutral")
                & ~focus.get("macro_tape_candidate", pd.Series(False, index=focus.index)).map(_truthy)
                & ~focus.get("macro_tape_direction", pd.Series("", index=focus.index)).astype(str).str.lower().isin(["bearish", "bullish"])
            )
            focus.loc[core_review, "candidate_source"] = "core_audit_review"
            focus.loc[core_review, "macro_tape_reason"] = "core watchlist review; neutral flow does not create a directional trade"
...
def _candidate_status_reason(row: Mapping[str, Any]) -> str:
    if _as_text(row.get("candidate_source")) == "core_audit_review":
        return "core watchlist reviewed; neutral/no-edge flow stays watch-only unless directional flow or macro tape improves"
...
def _unique_ordered_tickers(values: Iterable[Any]) -> list[str]:
    tickers: list[str] = []
    seen: set[str] = set()
    for value in values:
        ticker = _as_text(value).strip().upper()
...
def _agent_dispatch_prompt(lane: Mapping[str, Any], common_context: Mapping[str, Any], tasks: Sequence[Mapping[str, Any]]) -> str:
    ...
    return (
        ...
        "Return at least one review for every required review ticker before adding optional broader-universe "
        "reviews. Use the required ticker as the `ticker` value exactly once or more in your lane output. "
        "If evidence is mixed, return caution rather than omitting the ticker.\n"
...
def _closed_trade_strategy_route_metrics(root: Optional[Path]) -> dict[str, dict[str, Any]]:
    if root is None:
        return {}
    resolved_root = Path(root).expanduser().resolve()
    out_root = resolved_root if resolved_root.name == "out" else resolved_root / "out"
    actual = _actual_calibration_frame(resolved_root, out_root)
    ...
    *,
    ...
    for ticker in tickers:
        ...
        if decision:
            ...
            if _as_text(candidate.get("candidate_source")) == "core_audit_review" and _as_text(candidate.get("bias")) == "neutral":
...
def _coverage_reason(
    ...
    if candidate:
        ...
        if status == "NO_DIRECTIONAL_EDGE":
            ...
            return f"core watchlist reviewed; neutral flow bias in UW sources; score {score_text}; no directional spread should be forced"
    ...
    if actual_status == "PASS" and replay_status == "PASS":
        ...
        if current_ticket_count > 0:
            return (
                ...
                "Keep green eligibility conditional on live validation, sizing, review, and material-profit gates.",
...
def build_trade_tickets(decision_board: pd.DataFrame) -> pd.DataFrame:
    ...
    tickets = working[
        ...
        & (
            (not _v0_require_per_ticker_agent_review())
            |
...
def apply_goal_confidence_gate_to_decision_board(
    ...
    out.loc[ready_mask, "ready_to_enter"] = False
    out.loc[ready_mask, "target_order_status"] = "target_order_candidate"
    out.loc[ready_mask, "execution_status"] = "needs_confidence"
    out.loc[ready_mask, "execution_blockers"] = out.loc[ready_mask, "execution_blockers"].map(
    ...
    reason = (
        ...
        f"order_entry={summary.get('order_entry_confidence_rating', 0.0)}/10."
    )
    out.loc[ready_mask, "status_reason"] = out.loc[ready_mask, "status_reason"].map(
        lambda value: _append_reason(value, reason)
    )
    out.loc[ready_mask, "synthesis_reason"] = out.loc[ready_mask, "synthesis_reason"].map(
        lambda value: _append_reason(value, "Demoted from green because goal confidence gate is blocked.")
    )
    return out
...
def _negative_route_family_evidence_blocks_action(row: Mapping[str, Any]) -> bool:
    ...
    if _as_text(row.get("route_action")) != "construct_research_only_negative_family_evidence":
        return False
    sample = int(_as_float(row.get("route_evidence_sample_size")) or 0)
    if sample < MIN_EXPECTANCY_SAMPLE_SIZE:
        return False
    avg_pnl = _as_float(row.get("route_evidence_avg_pnl"))
    profit_factor = _as_float(row.get("route_evidence_profit_factor"))
    status = _as_text(row.get("route_evidence_status")).upper()
    return bool(
        status == "BLOCK"
...
def _target_order_status(
    ...
    *,
    ...
    if NEGATIVE_ROUTE_FAMILY_EVIDENCE_BLOCKER in blocker_set:
    ...
    if _profitability_calibration_replay_blocks_target_surface(row):
        return "review_only_profitability_calibration"
...
def build_expectancy_evidence(
    ...
    if broker_matched_outcomes is not None:
        rows.append(_expectancy_from_broker_matched_outcomes(broker_matched_outcomes, current_tickers))
...
def build_outcome_evidence_audit(
    ...
    *,
    ...
    if broker_matched_outcomes is not None:
        rows.append(_broker_matched_outcome_evidence_audit_row(broker_matched_outcomes, current_tickers=current_tickers))
    if as_of_date is not None:
        as_of_day = parse_as_of(as_of_date)
        for row in rows:
...
def summarize_outcome_evidence_audit(outcome_evidence_audit: pd.DataFrame) -> dict[str, Any]:
    ...
    forward_without_realized = audit[forward_mask & realized.le(0)].get("source", pd.Series(dtype=object)).astype(str).tolist()
    contributes = audit[
...
def build_broker_outcome_match_audit(root: Path) -> pd.DataFrame:
    ...
    if not _safe_non_v4_path(closed_path) or not closed_path.exists():
        return pd.DataFrame(
            [
                {
                    "match_source": "broker_closed_trades",
                    "match_status": "BLOCK",
                    "closed_trade_key": "",
                    "ticker": "",
                    "strategy": "",
                    ...
                    "blocker": "closed_trades_source_missing",
            ...
            [
                {
                    ...
                    "matched_run_ids": "",
                    "can_backfill_realized_pnl": False,
                    "blocker": "closed_trades_source_unreadable",
                    "source_path": str(closed_path),
...
def summarize_broker_outcome_match_audit(match_audit: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        "status": status_text,
        "rows": int(len(audit)),
        "closed_trade_rows": int(audit.get("closed_trade_key", pd.Series(dtype=object)).astype(str).nunique()),
...
def _ledger_row_symbol_keys(row: Mapping[str, Any]) -> list[str]:
    text = " ".join(
        _as_text(_mapping_get(row, key))
        for key in ("trade_key", "sell_leg", "buy_leg", "short_leg", "long_leg", "trade_plan")
        if _as_text(_mapping_get(row, key))
    )
...
def _broker_outcome_match_row(
    ...
    *,
    ...
    unique_matches = _unique_ledger_matches(matches)
    if not symbols:
        status = "BLOCK"
        ...
        status = "BLOCK"
        blocker = "no_exact_contract_match"
        can_backfill = False
        ...
        blocker = ""
        can_backfill = True
        note = "Unique exact contract match; this row is eligible for broker-confirmed realized P/L backfill."
    return {
        "match_source": match_source,
        "match_status": status,
...
def _closed_trades_outcome_evidence_audit_row(path: Path, *, current_tickers: set[str]) -> dict[str, Any]:
...
def _actual_forward_metrics_by_strategy_route(frame: pd.DataFrame) -> dict[str, dict[str, Any]]:
    if frame.empty or "strategy_route" not in frame.columns:
        return {}
...
def _target_order_candidate_count(decision_board: pd.DataFrame) -> int:
    if decision_board.empty or "target_order_status" not in decision_board.columns:
        return 0
    ready = (
        decision_board["ready_to_enter"].map(_truthy)
        if "ready_to_enter" in decision_board.columns
        else pd.Series(False, index=decision_board.index)
    )
    target = (
...
def _expectancy_from_broker_matched_outcomes(outcomes: pd.DataFrame, current_tickers: set[str]) -> dict[str, Any]:
    source = "broker_matched_outcomes"
    evidence_type = "broker_matched_recommendation_outcomes"
    path = Path("broker_matched_outcomes.csv")
    if outcomes is None or outcomes.empty:
        return _expectancy_missing_row(
            ...
            path,
            evidence_type,
            "No exact broker-to-recommendation matched outcomes are available.",
        )
    frame = outcomes.copy()
    source_paths = sorted(
        {
            part.strip()
            for value in frame.get("match_sources", pd.Series(dtype=object)).dropna().tolist()
            for part in _as_text(value).split(",")
            if part.strip()
        }
    )
    row = _expectancy_metrics_row(
        source,
        path,
        evidence_type,
        ...
        open_or_unrealized_count=0,
        note=(
            "Broker-confirmed exact contract matches to recommendation/execute ledgers. "
            "Diagnostic only: these rows are a de-duplicated subset of Schwab closed trades and must not be double-counted as independent actual evidence."
        ),
        status_override_func=_matched_outcome_expectancy_status,
    )
    row["source_path"] = ",".join(source_paths) or str(path)
    return row


def _matched_outcome_expectancy_status(sample: int, win_rate: float, avg_pnl: float, profit_factor: float) -> str:
    if sample <= 0:
        return "BLOCK"
    if (math.isfinite(avg_pnl) and avg_pnl <= 0) or (math.isfinite(profit_factor) and profit_factor < 1.0):
        return "BLOCK"
    return _expectancy_status(sample, win_rate, avg_pnl, profit_factor)
...
def _profitability_confidence_rating(
    ...
    if broker_matched_negative:
        blockers.append("broker_matched_recommendation_outcomes_negative")
        evidence.append(_evidence_row_brief(broker_matched_negative, "broker_matched_outcomes=negative"))
        rating = min(rating, 3.0)
    elif broker_matched_positive:
        evidence.append(_evidence_row_brief(broker_matched_positive, "broker_matched_outcomes=PASS_pipeline_attribution"))
    elif broker_matched_sampled:
        blockers.append("broker_matched_recommendation_outcomes_sample_too_small")
        evidence.append(_evidence_row_brief(broker_matched_sampled, "broker_matched_outcomes=insufficient_pipeline_attribution"))
        rating = min(rating, 6.0)
    else:
        blockers.append("broker_matched_recommendation_outcomes_missing")
        evidence.append("broker_matched_outcomes=no_exact_pipeline_attribution")
        rating = min(rating, 6.0)
...
def render_report(
    ...
    lines = [
        ...
        f"({outcome_audit_summary.get('realized_pnl_count', 0)} realized P/L rows; "
        f"forward gaps {outcome_audit_summary.get('forward_sources_without_realized_outcomes', [])})",
        f"- Broker outcome match audit: {broker_match_summary.get('status', 'unknown')} "
        f"({broker_match_summary.get('backfillable_rows', 0)} exact backfill rows; "
        f"{broker_match_summary.get('ambiguous_rows', 0)} ambiguous)",
        f"- Broker matched outcomes: {broker_matched_summary.get('status', 'unknown')} "
        ...
        f"- Outcome evidence audit: `{outcome_audit_path}`" if outcome_audit_path else "- Outcome evidence audit: `outcome_evidence_audit.csv`",
        f"- Broker outcome match audit: `{broker_match_path}`"
        if broker_match_path
        else "- Broker outcome match audit: `broker_outcome_match_audit.csv`",
        f"- Broker matched outcomes: `{broker_matched_path}`"
        if broker_matched_path
        else "- Broker matched outcomes: `broker_matched_outcomes.csv`",
...
def run_design_smoke(
    ...
    _write_csv(paths["outcome_evidence_audit"], OUTCOME_EVIDENCE_AUDIT_COLUMNS, [])
...
def _synthesis_score(
    ...
    if not _v0_late_evidence_gates_diagnostic_only() and _negative_strategy_expectancy_blocks_green(row):
        adjustment -= 30.0
        reasons.append("negative vertical-spread realized history blocks green -30")
    if (
        status in {RecommendationStatus.ENTER.value, RecommendationStatus.ENTER_WITH_PORTFOLIO_RISK.value}
        and not _v0_late_evidence_gates_diagnostic_only()
        and not _positive_strategy_expectancy_ready_for_green(row)
    ):
        adjustment -= 25.0
...
def _apply_live_debit_spread(
    ...
    *,
    ...
    existing_route = _as_text(row.get("strategy_route") or row.get("strategy"))
    route_reason = _as_text(row.get("route_reason"))
    if existing_route and existing_route != route:
        route_reason = (
            "bullish_core_defined_risk_upside_route"
            if route == "bull_call_debit"
            else "bearish_core_defined_risk_downside_route"
        )
    out.update(
        {
            "strategy": route,
