"""Leakage-safe, price-first rebuild of the UW pattern pipeline.
...
def quote_cache_path(base_dir: Path, kind: str, payload: Any) -> Path:
    cache_dir = base_dir / "out" / "pattern_rebuild_v1" / "cache"
...
def source_index_path(base_dir: Path, kind: str, ref: SourceRef) -> Path:
    ...
    path = base_dir / "out" / "pattern_rebuild_v1" / "cache" / "source_index" / kind / f"{fingerprint}.jsonl.gz"
...
def _managed_primary_report(
    ...
    lines = [
        ...
        f"- Review-only current candidates: **{metadata.get('current_review_position_count', len(review))}** (details in action_board.csv)",
    ...
    if not report_rows:
        lines.append(
            "| DO_NOT_TRADE | NO_LIVE_EXECUTION_CANDIDATE | - | - | - | - | - | - | - | - | No current ticket cleared the evidence gates |"
        )
    ...
    lines.extend(
        [
            ...
            "## Managed Validation",
            "",
            "| Strategy | Status | Train avg R | Validation avg R | Holdout avg R | Holdout PF | Holdout cal. lower | Holdout dates | Holdout coverage | Blockers |",
            "|---|---|---:|---:|---:|---:|---:|---:|---:|---|",
        ]
    )
    for row in validation:
        lines.append(
            "| {strategy_key} | {status} | {train_average_net_R} | {validation_average_net_R} | {holdout_average_net_R} | {holdout_profit_factor} | {holdout_calibration_score} | {holdout_unique_signal_dates} | {holdout_coverage} | {blockers} |".format(
                **{
                    key: str(row.get(key, "")).replace("|", "/")
                    for key in (
                        "strategy_key",
...
def _managed_ticket_state(
    ...
    is_qualified = evidence.get("status") == "QUALIFIED_MANAGED"
...
def run_managed_primary_pipeline(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    out_dir = (
        ...
        if args.out_dir
        else base_dir / "out" / "pattern_rebuild_v1" / requested_as_of
    )
    ...
    managed_result = run_managed_research(
        ...
        cache_root=base_dir / "out" / "pattern_rebuild_v1" / "cache" / "managed_quotes",
    )
    ...
    approved_board = [row for row in board if row.get("status") == "APPROVED_TRADE"]
    historical_board = [
        row for row in board if row.get("status") == "HISTORICAL_APPROVED_TRADE"
    ]
    review_board = [row for row in board if row.get("status") != "APPROVED_TRADE"]
    review_board = [
        row for row in review_board if row.get("status") != "HISTORICAL_APPROVED_TRADE"
    ]
    board = (
        approved_board
        + historical_board
        + review_board[: max(0, action_board_limit - len(approved_board) - len(historical_board))]
    )
    if not board:
        board = [
            {
    ...
    if not any(row.get("status") == "QUALIFIED_MANAGED" for row in validation):
    ...
    managed_metadata.update(
        {
            ...
            "current_execution_candidate_count": len(approved_board),
            "historical_validated_entry_count": len(historical_board),
...
def run_pipeline(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    out_dir = Path(args.out_dir).expanduser().resolve() if args.out_dir else base_dir / "out" / "pattern_rebuild_v1" / as_of
    out_dir.mkdir(parents=True, exist_ok=True)
    managed_research_dir = out_dir / "managed_research"
    ...
    if not getattr(args, "skip_managed_research", False):
        ...
        except Exception as exc:  # pragma: no cover - source failures are reported as artifacts
            managed_status = "FAILED"
            managed_error = f"{type(exc).__name__}: {exc}"
            managed_research_dir.mkdir(parents=True, exist_ok=True)
            (managed_research_dir / "managed_research_error.json").write_text(
                json.dumps(
                    {"status": managed_status, "error": managed_error, "no_order_placement": True},
                    indent=2,
                    sort_keys=True,
                ),
                encoding="utf-8",
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="python3 -m uwos.pattern_rebuild_v1")
