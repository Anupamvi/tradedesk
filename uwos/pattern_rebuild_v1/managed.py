"""Leakage-safe managed-exit option research for the pattern rebuild.
...
def named_mover_audit(
    ...
    from .core import ENERGY_PROXY_TICKERS, PRIMARY_TARGET_TICKERS
...
def _random_control_names(
    ...
    for sector, names in selected.items():
        pool = eligible.loc[eligible["signal_sector"].eq(sector), "ticker"].astype(str).unique()
...
def managed_selection_audit(
    ...
    for candidate_key, trades in sorted(trades_by_candidate.items()):
        work = trades.copy()
        ...
        final_calibration = calibration_summary.get(("signal", final_sample), {})
...
def run_managed_research(
    ...
    for key, trades in results.items():
        ...
        write_managed_artifacts(
            ...
            rows,
            {
                "pipeline": "pattern_rebuild_v1_managed",
                "strategy_key": key,
                "control": control,
    ...
    for candidate_key, candidate_trades in selection_results.items():
        ...
        (candidate_dir / "managed_exit_metadata.json").write_text(
            json.dumps(
                {
                    "pipeline": "pattern_rebuild_v1_managed_selection",
                    "candidate_key": candidate_key,
                    "selection_end": selection_end,
                    "final_holdout_start": holdout_start,
                    "holdout_used_for_selection": False,
