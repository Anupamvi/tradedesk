"""PD ranking for TRADE CSPs. Does not change conf, gates, or TRADE membership."""
...
from wheelo.config import CONTRACT_MULTIPLIER
from wheelo.num import fmt, to_float
...
def economics(cand: dict) -> tuple:
    prem = cand.get("premium") if isinstance(cand.get("premium"), dict) else {}
    strike = to_float(prem.get("csp_strike") or cand.get("csp_strike"))
    bid = to_float(prem.get("csp_bid") or prem.get("csp_premium") or cand.get("csp_bid"))
    if strike is None or bid is None:
        return None, None, None
    ml = max(0.0, strike - bid) * float(CONTRACT_MULTIPLIER)
    reward = bid * float(CONTRACT_MULTIPLIER)
    return ml, reward, ml


def _liquidity(cand: dict) -> Optional[float]:
    prem = cand.get("premium") if isinstance(cand.get("premium"), dict) else {}
    sizes = []
    for key in ("put_bid_size", "put_ask_size", "bid_size", "ask_size"):
        v = to_float(prem.get(key) if prem.get(key) is not None else cand.get(key))
        if v is not None:
            sizes.append(v)
    if sizes:
        return min(sizes)
    oi = to_float(prem.get("put_oi") if prem.get("put_oi") is not None else cand.get("put_oi") or cand.get("liquidity_lots"))
    return oi


def attach_trade_pd(cand: dict, now=None) -> dict:
    if not isinstance(cand, dict) or cand.get("conf_label") != "TRADE":
        return cand
    prem = cand.get("premium") if isinstance(cand.get("premium"), dict) else {}
    ml, reward, risk = economics(cand)
    if cand.get("max_loss_1lot") is not None:
        ml = to_float(cand.get("max_loss_1lot"))
    if cand.get("planned_reward") is not None:
        reward = to_float(cand.get("planned_reward"))
    if cand.get("planned_risk") is not None:
        risk = to_float(cand.get("planned_risk"))
    age = quote_age_seconds(
        quote_time_ms=prem.get("quote_time_ms") or cand.get("quote_time_ms"),
        quote_date=prem.get("quote_date") or cand.get("quote_date"),
        explicit=cand.get("quote_age_sec") or prem.get("quote_age_sec"),
        now=now,
    )
    pack = compute_pd(
        max_loss=ml,
        ...
        planned_risk=risk,
        liquidity_lots=_liquidity(cand),
        quote_age_sec=age,
        spread_frac=prem.get("spread_pct") if prem.get("spread_pct") is not None else cand.get("spread_frac"),
        size=cand.get("pd_size") if cand.get("pd_size") is not None else cand.get("contracts"),
    )
    stamp_pd(cand, pack)
    cand.update(pd_cells(cand))
    return cand
