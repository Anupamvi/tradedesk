"""Schwab tape: quotes, history, chains. GET only. No /orders."""
...
from wheelo.envload import schwab_credentials
...
def _parse_quote_wrap(key: str, wrap: dict) -> Optional[dict]:
    if not isinstance(wrap, dict):
        return None
    q = wrap.get("quote") if isinstance(wrap.get("quote"), dict) else wrap
    fund = wrap.get("fundamental") if isinstance(wrap.get("fundamental"), dict) else {}
    last = to_float(q.get("lastPrice")) or to_float(q.get("mark")) or to_float(q.get("closePrice"))
    if last is None:
        return None
    pe = to_float(q.get("peRatio")) or to_float(fund.get("peRatio"))
    div = to_float(q.get("divYield")) or to_float(fund.get("divYield"))
    return {
        "ticker": str(key).upper(),
        "last": last,
        "mark": to_float(q.get("mark")) or last,
        "bid": to_float(q.get("bidPrice")),
        "ask": to_float(q.get("askPrice")),
        "volume": to_float(q.get("totalVolume")),
        "pe_ratio": pe,
        "div_yield": div,
        "high52": to_float(q.get("52WkHigh")) or to_float(q.get("high52Week")),
        "low52": to_float(q.get("52WkLow")) or to_float(q.get("low52Week")),
        "description": str(q.get("description") or wrap.get("reference", {}).get("description") or ""),
    }
...
def quotes_many(tickers, asof: str) -> Dict[str, dict]:
    ...
    for i in range(0, len(names), 20):
        ...
        for key, wrap in payload.items():
            parsed = _parse_quote_wrap(key, wrap if isinstance(wrap, dict) else {})
            if parsed:
...
def option_quotes(symbols) -> Dict[str, dict]:
    token = _access_token()
    if not token:
        return {}
    ...
    for i in range(0, len(names), 10):
        ...
        for key, wrap in payload.items():
            ...
            out[str(key)] = {
                "bid": bid,
                "ask": ask,
                "mark": mark if mark is not None else last,
                "last": last,
            }
    return out
