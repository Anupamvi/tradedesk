# Assumption audit — 2026-09-16

Tape + evidence for grok-option v3.13. Unverified numbers stay unverified. Next AUDIT: when the journal's 20-trade rolling expectancy contradicts a rule, rewrite this file and bump the Assumptions-in-force footnote.
...
Bar $10k/month ($120k/year average, not a Calm floor). Account live Schwab $715k. Score all five structures; long stock does not veto a call wing or condor. No saved ticker veto list. **Name calendar before Expert** (IR > estimates; dated event parks that wing). Score ≠ P(win). Conf = quoted-delta naive POP + book win rate. Table dollars = 1 lot; Rec lots from Conf then sleeve cap then BP. Among gates, pick **credit/width then dollars** (not 15-wide scraps). Fire without flow is not an Expert row. Shield 1.0% then BP, aggregate 6–8%, **14–60 DTE** (not one monthly), max 4 one per sector. Cheap-vol Shield **0.12 / 0.22Δ / 0.80σ** in Calm and Normal. Event = that cluster / that expiry / that wing. Do not size up to chase. Do not restore empty 0.20. Manage 60–65% / 2×.
