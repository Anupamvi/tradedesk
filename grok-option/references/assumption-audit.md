# Assumption audit — 2026-09-17

Tape + evidence for grok-option v3.14. Unverified numbers stay unverified. Next AUDIT: when the journal's 20-trade rolling expectancy contradicts a rule, rewrite this file and bump the Assumptions-in-force footnote.

## Tape + Evidence (2026-09-17)

Market-open SCAN put **DELL Oct-16 510/500** on Expert because hardware was empty and frac **0.205** won the picker. VIX **15.80 Calm**. DELL ATM straddle **$82** on spot **~$584**. Name printed a sourced squeeze: Sep 1 earnings ($7.04 vs ~$4.9) from **~$425 → ~$574** in 16 sessions. ⚠️ worse-fill. Rec **7**. That is name-level shock vol dressed as Calm Shield. Highest-frac empty-sector is not a research pass. DELL → Review. Hardware empty is valid. Do not pad with AAPL.

## Tape + Evidence (2026-09-16)

Market-open SCAN printed TSLA Oct-16 **405/415C** inside an IC (frac 0.12, Δ 0.194, 1.28σ) as Expert. Tesla had already dated **Roadster unveil Oct 1** (company 9/12) and Q3 deliveries land in the first days of October; Goldman 9/16 cut Q3 deliveries 490k → 435k. October monthly-return folklore is not the issue (Oct up ~1 in 3 years after 2021). The miss was **no name-calendar pass before Expert**.
Same card: UNH Oct-16 put Expert on aggregator **est. Oct 27**. Company IR **2026-09-15**: Q3 results **Oct 13 BMO**. That is earnings-overlap. IR beats estimates.
Fix: name calendar is a SCAN step. Park the threatened wing. Company IR date in the life is overlap.
...
| 14 | No new Core credit inside 7 days of earnings | **AMEND** | A 7-day entry window still lets a 30–45 DTE sit through the print. Replacement: skip unless a confirmed earnings date exists and **expiry_date < earnings_date**. **Company IR beats aggregator estimates** (UNH 9/15 PR = Oct 13 BMO, not est. Oct 27). Unknown date = skip. Open lines that would live into the print: close or roll in front of it. |
...
| 35 | Hardcoded “core longs” never get call credits or iron condors | **KILL** | 2026-08-31: operator holds the stock and the spread; decides later to close the spread or sell shares. A ticker list is over-gating (same class as #30). Replacement: no protected-name list. Call credit / condor if geometry + earnings + **name calendar** + Event/Crowded + quotes pass. Shares held → Notes tag, close rather than deliver unless asked. Naked shorts stay banned. |
| 36 | Geometry-pass is enough for Expert; skip name-catalyst research | **KILL** | 2026-09-16: TSLA 405C Expert into a Tesla-dated Oct 1 unveil + Q3 deliveries; UNH Expert on est. Oct 27 after IR set Oct 13 BMO. Replacement: **name calendar before Expert**. Web-source dated events in `(scan, expiry]`. Seasonality folklore is not a veto. A sourced event parks **that wing**; IC becomes the other vertical or skip. Company IR > aggregator. Missing this pass is incomplete. |
| 37 | Highest frac in an empty sector is Expert | **KILL** | 2026-09-17: DELL 510/500 Expert on frac 0.205 after a sourced Sep-1 squeeze (~$425 → ~$574, ATM straddle $82 vs VIX 15.8 Calm), ⚠️, Rec 7. Replacement: ⚠️ + sourced vertical squeeze stays **Review**. Empty sector is valid. Do not pad. Credit/width still ranks among names that already clear tape quality. |

## In force until the next 20-trade review

Bar $10k/month ($120k/year average, not a Calm floor). Account live Schwab $715k. Score all five structures; long stock does not veto a call wing or condor. No saved ticker veto list. **Name calendar before Expert** (IR > estimates; dated event parks that wing). **⚠️ + sourced vertical squeeze is Review, not a 4th-slot pad.** Score ≠ P(win). Conf = quoted-delta naive POP + book win rate. Table dollars = 1 lot; Rec lots from Conf then sleeve cap then BP. Among gates, pick **credit/width then dollars** (not 15-wide scraps). Fire without flow is not an Expert row. Shield 1.0% then BP, aggregate 6–8%, **14–60 DTE** (not one monthly), max 4 one per sector. Cheap-vol Shield **0.12 / 0.22Δ / 0.80σ** in Calm and Normal. Event = that cluster / that expiry / that wing. Do not size up to chase. Do not restore empty 0.20. Manage 60–65% / 2×.
