...
9. **Write the card to disk** — `CODE/out/grok-option/YYYY-MM-DD/GROK_OPTION.md` from `daily-card.md`. Chat is the file link, not a substitute. Executable table first. 1-lot max profit / 1-lot max loss / Rec lots from Conf. Keep Conf; add PD, N, R_cons, L; sort Expert PD desc (nulls last). Print `PD sort only — conf unchanged.` Sleeve board still lists all six. Then Shock watch, then sleeve board, then **Cleared structures — review** with a full row for every geometry-pass (legs, net, Conf, Rec lots, Status). Plain markdown, no HTML. Never put trades under “not executable.” Never mix 1-lot P/L with N-lot dollars. Then 3–5 **Assumptions in force** bullets.
